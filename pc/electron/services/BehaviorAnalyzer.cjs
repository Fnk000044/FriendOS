...
<<<<<<< HEAD
  ...
  let baseline = null;
  if (records.length >= MIN_BASELINE_DAYS) {
    const raw = calculatePersonalBaseline(records);
    if (raw && raw.hasBaseline) {
      baseline = {
        hasBaseline: true,
        daysUsed: raw.daysUsed,
        mood: raw.mood,
        taskCompletion: raw.taskCompletion,
        habitConsistency: raw.habitConsistency,
        diaryFrequency: raw.diaryFrequency,
      };
    }
  }

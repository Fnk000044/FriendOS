...
function generateSuggestions(profile) {
  ...
  if (profile.riskLevel === 'high' || profile.riskLevel === 'critical') {
    suggestions.push('与信任的人分享你的感受，不要一个人扛着');
    suggestions.push('尝试 4-7-8 呼吸：吸气 4 秒、屏息 7 秒、呼气 8 秒');
    suggestions.push('建议拨打心理援助热线：400-161-9995 或 12356');
    suggestions.push('把让你难受的事情写下来，写完再撕掉也是一种释放');
    suggestions.push('今天先完成一件最小的事（比如起床喝杯水），给自己一个支点');
  } else {
    // ── 情绪维度 ───────────────────────────────────────────
    if (d.mood != null && d.mood < 45) {
      suggestions.push('记录一件今天让你有一点点开心的小事');
      suggestions.push('听一首老歌或看一段搞笑视频，给情绪一个出口');
    } else if (d.mood != null && d.mood > 75) {
      suggestions.push('状态不错！把这份能量用在最重要的那件事上');
    }

    // ── 压力维度 ───────────────────────────────────────────
    if (d.stress != null && d.stress > 60) {
      suggestions.push('把担心的事列出来，区分"能控制的"和"不能控制的"');
      suggestions.push('做一次 4-7-8 呼吸练习，两分钟让心跳慢下来');
      suggestions.push('给自己设一个"放下手机"时段，哪怕只有 20 分钟');
    }

    // ── 睡眠/作息维度 ──────────────────────────────────────
    if (d.sleep != null && d.sleep < 50) {
      suggestions.push('今晚提前 30 分钟上床，睡前不看屏幕');
      suggestions.push('把睡觉时间固定下来，身体会慢慢习惯这个节律');
    }

    // ── 社交维度 ───────────────────────────────────────────
    if (d.social != null && d.social < 35) {
      suggestions.push('给一位朋友发条消息，哪怕只是"最近怎么样"');
      suggestions.push('参加一次线下活动或约人散步，社交是情绪的养分');
    }

    // ── 精力/自护维度 ──────────────────────────────────────
    if (d.energy != null && d.energy < 40) {
      suggestions.push('保持规律作息和适度运动，快走 15 分钟就有效');
    }
    if (d.selfCare != null && d.selfCare < 40) {
      suggestions.push('给自己安排一段"什么都不做"的时间，这很重要');
    }

    // ── 中风险补充 ─────────────────────────────────────────
    if (profile.riskLevel === 'medium') {
      suggestions.push('尝试写一篇感恩日记，记录今天值得感恩的事');
      suggestions.push('与朋友或家人聊聊最近的生活');
    }
  }

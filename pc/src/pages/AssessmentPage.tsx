...
export default function AssessmentPage() {
  ...
  const handleComplete = async (type: ScaleType, scores: number[]) => {
    // 验证分数范围
    ...
    if (type !== 'ISI7' && type !== 'CDRISC10') {
      ...
      // ISI-7 / CD-RISC-10：0-4 五级
      if (scores.some(s => typeof s !== 'number' || s < 0 || s > 4 || !Number.isInteger(s))) {
    ...
    if (type === 'PHQ9') {
      ...
      else if (totalScore <= 14) level = 'subclinical';
      else if (totalScore <= 21) level = 'moderate';
      else level = 'severe';
    } else if (type === 'CDRISC10') {
      // CD-RISC-10: Campbell-Sills & Stein, 2007（分数越高韧性越强）
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      {view === 'home' && (
        <div className="space-y-4">
          ...
          >
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-text-primary">ISI-7 失眠严重程度</h3>
              <p className="text-sm text-text-muted">7 个问题，评估最近两周的失眠严重程度（Bastien 2001）</p>
            </div>
            {isi7History.length > 0 && (
              <span className="text-xs text-text-muted">
                上次：{isi7History[0].totalScore}分
              </span>
            )}
          </Card>
          ...
          >
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-text-primary">CD-RISC-10 心理韧性</h3>
              <p className="text-sm text-text-muted">10 个问题，评估压力下的心理复原力（Connor-Davidson 10 题版）</p>
            </div>
            {cdriscHistory.length > 0 && (
              <span className="text-xs text-text-muted">
                上次：{cdriscHistory[0].totalScore}分
              </span>
            )}
          </Card>
      ...
      {view === 'isi7' && (
        <Card>
          <ISI7Form
            onComplete={(scores) => handleComplete('ISI7', scores)}
            onCancel={() => setView('home')}
          />
        </Card>
      )}
      ...
      {view === 'cdrisc10' && (
        <Card>
          <CDRISC10Form
            onComplete={(scores) => handleComplete('CDRISC10', scores)}
            onCancel={() => setView('home')}
          />
        </Card>
      )}
      ...
      {view === 'history' && (
        <div className="space-y-4">
              ...
              {isi7History.map(a => (
                <Card key={a.id} className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="ml-3 font-semibold">{a.totalScore} 分</span>
                    <span className="ml-2 text-sm text-text-secondary">
                      ({a.level === 'none' ? '无临床显著失眠' : a.level === 'subclinical' ? '亚临床失眠' : a.level === 'moderate' ? '中度临床失眠' : '重度临床失眠'})
                    </span>
                  </div>
                  <button
                    onClick={() => deleteAssessment(a.id)}
          ...
          {cdriscHistory.length > 0 && (
            <div className="space-y-2">
              <h3 className="font-medium text-text-primary">CD-RISC-10 心理韧性记录</h3>
              {cdriscHistory.map(a => (
                <Card key={a.id} className="flex items-center justify-between">
                  <div>
                    <span className="text-sm text-text-muted">{a.date}</span>
                    <span className="ml-3 font-semibold">{a.totalScore} 分</span>
                    <span className="ml-2 text-sm text-text-secondary">
                      ({a.level === 'low' ? '韧性偏低' : a.level === 'moderate' ? '中等' : '良好'})
                    </span>
                  </div>
                  <button
                    onClick={() => deleteAssessment(a.id)}

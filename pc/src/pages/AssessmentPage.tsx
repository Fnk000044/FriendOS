...
type View = 'home' | 'phq9' | 'gad7' | 'pss10' | 'result' | 'history';
...
export default function AssessmentPage() {
  ...
  const [currentType, setCurrentType] = useState<'PHQ9' | 'GAD7' | 'PSS10'>('PHQ9');
  const [lastResult, setLastResult] = useState<Assessment | null>(null);

  const handleComplete = async (type: 'PHQ9' | 'GAD7' | 'PSS10', scores: number[]) => {
    // 验证分数范围
    const expectedLength = type === 'PHQ9' ? 9 : type === 'GAD7' ? 7 : 10;
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      {view === 'home' && (
        <div className="space-y-4">
          ...
          {(phq9History.length > 0 || gad7History.length > 0 || pss10History.length > 0) && (
      ...
      {view === 'result' && lastResult && (
        <Card>
          <AssessmentResult
            ...
            onRetake={() => setView(currentType === 'PHQ9' ? 'phq9' : currentType === 'GAD7' ? 'gad7' : 'pss10')}
          />
        </Card>
      )}

...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {/* 本地个性化校准（知己度）重置 */}
      <Card>
        <h3 className="text-sm font-semibold text-text-primary mb-1">{t('selfevo.reset')}</h3>
        <p className="text-xs text-text-muted mb-3">{t('selfevo.reset_desc')}</p>
        <div className="flex items-center gap-3">
          <Button variant="secondary" size="sm" onClick={handleResetCalibration}>
            {t('selfevo.reset')}
          </Button>
          {calibReset && (
            <span className="text-xs text-emerald-600 dark:text-emerald-400">{t('selfevo.reset_success')}</span>
          )}
        </div>
      </Card>

...
<<<<<<< HEAD
  ...
  // 因此 !hasKey 覆盖「无 Key / IPC 异常」两种场景，一律快速降级。
  let providerConfig: ChatProviderConfig | null = null;
  try {
    providerConfig = await API.chatGetProviderConfig();
  } catch {
    providerConfig = null;
  }
  if (!providerConfig?.hasKey) {
    const fb = await API.chatFallback({
      text,
      emotionLabel,
      negativeProb: sentiment?.negativeProb,
      positiveProb: sentiment?.positiveProb,
      crisisProb: sentiment?.crisisProb,
    ...
    useChatStore.getState().setProviderStatus('fallback');
    if (fb.isCrisis) {
      useCrisisStore.getState().show('critical', 'chat', text);
    }
...
export async function listSessions() {
  try {
    ...
    console.error('[ChatService] listSessions error:', e);
    return [];
  }
}

/**
 * 新开一个对话：清空当前消息与会话状态，切换到新 sessionId
 */
...
export async function deleteSession(sessionId: string) {
  try {
    ...
    console.error('[ChatService] deleteSession error:', e);
  }
}

/**
 * 切换会话：加载指定会话消息并更新当前 sessionId
 */

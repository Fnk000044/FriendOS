...
export default function RiskTimelineChart({ days = 30 }: Props) {
  ...
  const autoSummary = useMemo(() => {
    ...
    const slope = pts[n - 1] - pts[0];
    const direction = Math.abs(slope) <= 3 ? 'stable' : slope > 0 ? 'rising' : 'falling';
    const level = mean >= 60 ? 'elevated' : mean >= 40 ? 'attention' : 'normal';
    return { direction, level, latest: pts[n - 1] };
  }, [data]);

  const summaryText = autoSummary
    ? t(
        autoSummary.direction === 'rising'
          ? 'risk.timeline_summary_rising'
          : autoSummary.direction === 'falling'
            ? 'risk.timeline_summary_falling'
            : 'risk.timeline_summary_stable',
        { level: t(autoSummary.level === 'elevated' ? 'risk.status_high' : autoSummary.level === 'attention' ? 'risk.status_attention' : 'risk.status_normal'), latest: autoSummary.latest },
      )
    : null;

  return (
    <div className="glass-card rounded-2xl p-6">
      <h3 className="text-lg font-semibold text-text-primary mb-1">风险时间线</h3>
      ...
      {/* 自动解读 */}
      {summaryText && (
        <p className="text-xs text-text-secondary mt-2 text-center" style={{ background: 'var(--bg-hover)' }}>
          💡 {summaryText}
        </p>
      )}
    </div>

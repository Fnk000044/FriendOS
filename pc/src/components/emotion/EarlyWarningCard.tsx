...
interface EarlyWarningCardProps {
  /** 风险计算完整结果（用于证据链下钻；不传则隐藏证据链入口） */
  riskResult?: (RiskResultLike & { riskLevel: string; summary?: string }) | null;
  hasData?: HasDataMap;
}

export default function EarlyWarningCard({ riskResult, hasData }: EarlyWarningCardProps) {
  const { t } = useLanguage();
  ...
  const handleOpenChain = () => {
    ...
    setChain(
      buildEvidenceChain(
        {
          totalScore: riskResult.totalScore ?? 0,
